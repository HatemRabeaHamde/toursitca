...
@section('page-subtitle', 'Featured destination cards shown on the homepage')

@section('content')

<div x-data="destinationsManager()" x-init="load()" class="adm-grid-3-2">

    {{-- Add form --}}
    <div>
        <div class="adm-card">
            <div class="adm-card-header"><span class="adm-card-title">Add Destination</span></div>
            <div class="adm-card-body" style="display:flex;flex-direction:column;gap:14px;">
                ...
                <div class="adm-form-row" style="margin:0;">
                    ...
                    <input type="text" x-model.trim="form.slug" class="adm-input" placeholder="marrakech" required>
                </div>
                <div class="adm-form-row" style="margin:0;">
                    <label class="adm-label">Name (EN) <span style="color:var(--deal);font-size:.7rem;">*</span></label>
                    <input type="text" x-model.trim="form.name_en" class="adm-input" placeholder="Marrakech" required>
                </div>
                <div class="adm-form-row" style="margin:0;">
                    <label class="adm-label">Name (FR)</label>
                    <input type="text" x-model="form.name_fr" class="adm-input" placeholder="Marrakech">
                </div>
                <div class="adm-form-row" style="margin:0;">
                    <label class="adm-label">Name (NL)</label>
                    <input type="text" x-model="form.name_nl" class="adm-input" placeholder="Marrakesh">
                </div>
                <div class="adm-form-row" style="margin:0;">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                    ...
                    <div class="adm-form-row" style="margin:0;">
                        <label class="adm-label">Featured</label>
                        <select x-model="form.is_featured" class="adm-select">
                ...
                <button @click="save()" :disabled="saving" class="adm-btn adm-btn--primary">
                    <span x-show="!saving">Add Destination</span>
    ...
    <div class="adm-card">
        <div class="adm-card-header">
            <span class="adm-card-title">All Destinations</span>
        ...
        <div x-show="!loading && items.length === 0" class="adm-empty"><p>No destinations yet.</p></div>
                        ...
                        <tr>
                            ...
                            <td>
                                <div x-show="editingId !== item.id">
                                    <div style="font-weight:600;font-size:.875rem;color:var(--adm-text);" x-text="item.name?.en || item.name || '—'"></div>
                                ...
                                <div x-show="editingId === item.id" style="display:flex;flex-direction:column;gap:5px;">
                                    ...
                                    <div style="display:flex;gap:6px;">
                                        <input type="number" x-model.number="editForm.sort_order" class="adm-input" placeholder="Sort" style="font-size:.8rem;padding:5px 8px;width:60px;">
                                        <select x-model="editForm.is_featured" class="adm-select" style="font-size:.8rem;padding:5px 8px;">
                            ...
                            <td>
                                <div style="display:flex;flex-direction:column;gap:3px;">
                                    <span x-show="item.is_featured" class="adm-badge adm-badge--confirmed" style="font-size:.65rem;">Featured</span>
...
function destinationsManager() {
    return {
        ...
        form: { slug:'', name_en:'', name_fr:'', name_nl:'', city:'', image_url:'', sort_order:0, is_featured:'0', is_active:'1' },
        ...
        async load() {
            this.loading = true;
            try {
                const res = await fetch('{{ route('admin.landing.destinations.index') }}', {
        ...
        payload(f, originalSlug) {
            return {
                slug: originalSlug ?? f.slug,
                name: { en: f.name_en, fr: f.name_fr || f.name_en, nl: f.name_nl || f.name_en },
                city: f.city,
                image_url: f.image_url || null,
                sort_order: f.sort_order ?? 0,
                is_featured: f.is_featured === '1' || f.is_featured === true,
        ...
        async save() {
            ...
            this.saving = true;
            try {
                const res = await fetch('{{ route('admin.landing.destinations.store') }}', {
                    method: 'POST',
                ...
                if (!res.ok) {
                    ...
                    this.form = { slug:'', name_en:'', name_fr:'', name_nl:'', city:'', image_url:'', sort_order:0, is_featured:'0', is_active:'1' };
                    this.showToast('Destination added.');
                }
        ...
        startEdit(item) {
            ...
            this.editForm = { name_en: item.name?.en??'', name_fr: item.name?.fr??'', name_nl: item.name?.nl??'', city: item.city??'', image_url: item.image_url??'', sort_order: item.sort_order??0, is_featured: item.is_featured?'1':'0', is_active: item.is_active?'1':'0' };
        },
        ...
        async update(item) {
            ...
            this.saving = true;
            try {
                const res = await fetch(`{{ url('admin/landing/destinations') }}/${item.id}`, {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-CSRF-TOKEN': this.csrf() },
                ...
                if (res.ok) {
                    ...
                    this.cancelEdit(); this.showToast('Destination updated.');
                } else {
                    const err = await res.json();
                    this.formErrors = Object.values(err.errors ?? {e: ['Server error']}).flat();
                }
        ...
        async remove(item) {
            ...
            try {
                const res = await fetch(`{{ url('admin/landing/destinations') }}/${item.id}`, { method: 'DELETE', headers: { 'Accept': 'application/json', 'X-CSRF-TOKEN': this.csrf() } });
                if (!res.ok) { this.showToast('Could not delete destination.', false); return; }
