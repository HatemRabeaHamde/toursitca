...
<article class="xc">
    <div class="xc-img">
        <img src="{{ $card->thumbnailUrl ?: 'https://images.unsplash.com/photo-1509316785289-025f5b846b35?w=1400&q=85&auto=format&fit=crop' }}"
             alt="{{ $card->title }}"
             loading="lazy"
             onerror="this.onerror=null;this.src='https://images.unsplash.com/photo-1509316785289-025f5b846b35?w=1400&q=85&auto=format&fit=crop';">
        ...
        <button
            type="button"
            class="xc-wish"
            aria-label="{{ __('ui.labels.save') }}"
            x-data="wishlistBtn({
                isSaved: {{ ($wishlisted ?? []) && in_array($card->id, $wishlisted ?? []) ? 'true' : 'false' }},
                toggleUrl: '{{ route('site.wishlist.toggle', ['locale' => app()->getLocale(), 'experience' => $card->id]) }}'
            })"
            :class="{ 'is-saved': isSaved }"
            @click.prevent.stop="toggle()"
        >
            <svg width="17" height="17" viewBox="0 0 24 24" :fill="isSaved ? 'currentColor' : 'none'" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
        </button>
    </div>
