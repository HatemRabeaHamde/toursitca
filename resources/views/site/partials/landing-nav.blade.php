...
<nav data-landing-nav data-solid="{{ $solid ? 'true' : 'false' }}" @class(['scrolled' => $solid])>
    <a href="{{ $homeUrl }}" class="nav-logo">{{ config('brand.name_first') }}<em>{{ config('brand.name_second') }}</em></a>
    ...
    <div class="nav-right">
        ...
        @auth
            <a href="{{ route('site.dashboard.bookings', ['locale' => $locale]) }}" class="nav-login">{{ __('ui.nav.dashboard') }}</a>
        @else
...
<div class="nav-mobile" :class="{ 'is-open': menuOpen }" @click.self="menuOpen = false">
    <div class="nav-mobile-panel">
        ...
        @auth
            <a href="{{ route('site.dashboard.bookings', ['locale' => $locale]) }}" class="nav-mobile-login" @click="menuOpen = false">{{ __('ui.nav.dashboard') }}</a>
        @else
