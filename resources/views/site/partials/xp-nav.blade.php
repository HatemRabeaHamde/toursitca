@php
    ...
    $searchNav ??= [];
    $experienceForNav = $experience ?? null;
    $destination = $searchNav['destination'] ?? ($experienceForNav?->location_city ?? '');
    $currentFilters ??= [];
    $searchValue = $currentFilters['search'] ?? request('search', '');
    $cityValue = $currentFilters['city'] ?? request('city', $destination);
    $dateFromValue = $currentFilters['date_from'] ?? request('date_from', request('date', ''));
    $dateToValue = $currentFilters['date_to'] ?? request('date_to', '');
    $participantsValue = max(1, (int) ($currentFilters['participants'] ?? request('participants', 1)));
    ...
    $participantsValue = min($participantsValue, $maxParticipants);
    $languageUrl = function (string $loc) use ($experienceForNav): string {
        if ($experienceForNav) {
            return route('site.experiences.show', ['locale' => $loc, 'experience' => $experienceForNav->slug]);
        }

        $query = request()->except(['locale', 'page']);
        $url = route('site.experiences.index', ['locale' => $loc]);
...
<nav class="xp-nav"
     ...
     x-data="{
         activePanel: null,
         searchQuery: @js($searchValue ?: $cityValue ?: ''),
         selectedCity: @js($cityValue ?: ''),
         cities: {{ $citiesJson }},
         suggestions: {{ $suggestionsJson }},
         startDate: @js($dateFromValue ?: ''),
         endDate: @js($dateToValue ?: ''),
         adults: {{ $participantsValue }},
         children: 0,
         monthCursor: new Date(new Date().getFullYear(), new Date().getMonth(), 1),
         maxParticipants: {{ $maxParticipants }},
         get isOpen() { return this.activePanel !== null; },
    ...
    <div class="xp-nav-backdrop" x-show="isOpen" x-cloak x-transition.opacity @click="close()"></div>

    <a href="{{ $homeUrl }}" class="xp-nav-logo" aria-label="{{ config('app.name') }}">
        <span>{{ config('brand.name_first') }}</span><em>{{ config('brand.name_second') }}</em>
    </a>

    <form method="GET" action="{{ $searchUrl }}" class="xp-nav-search" :class="{ 'is-active': isOpen }">
        ...
        <div class="xp-nav-panel xp-nav-panel--search" x-show="activePanel === 'search'" x-cloak x-transition>
            <div class="xp-nav-panel-section">
                <p>{{ __('ui.experience_detail.nav_search.recent_searches') }}</p>
                <button type="button" class="xp-nav-recent" @click="selectCity(@js($destination))">
                    <span aria-hidden="true">◷</span>
                    <strong>{{ $destination ?: __('ui.landing.morocco') }}</strong>
                </button>
            </div>
    ...
    <div class="xp-nav-right">
        @if (count($locales) > 1)
            <div class="xp-nav-lang" x-data="{ open: false }" @click.outside="open = false">
                <button type="button" class="xp-nav-icon-btn" @click="open = ! open" :aria-expanded="open.toString()">
                    {{ strtoupper($locale) }}
                </button>
                <div class="xp-nav-lang-menu" x-show="open" x-cloak x-transition.opacity>
                    @foreach ($locales as $loc => $meta)
                        <a href="{{ $languageUrl($loc) }}"
                           class="xp-nav-lang-item {{ $locale === $loc ? 'is-active' : '' }}">
                            {{ strtoupper($loc) }} <span>{{ $meta['native'] }}</span>
                        </a>
                    @endforeach
                </div>
            </div>
        ...
        @auth
            <a href="{{ route('site.dashboard.bookings', ['locale' => $locale]) }}" class="xp-nav-icon-link">{{ __('ui.nav.dashboard') }}</a>
        @else
            <a href="{{ route('login') }}" class="xp-nav-auth-link">{{ __('ui.nav.login') }}</a>
            <a href="{{ route('register') }}" class="xp-nav-auth-cta">{{ __('ui.nav.register') }}</a>
        @endauth
    </div>
</nav>
